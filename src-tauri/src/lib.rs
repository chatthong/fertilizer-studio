...
#[tauri::command]
fn variant_detail(state: tauri::State<Db>, variant_id: String) -> Result<Option<db::VariantDetail>, String> {
    let conn = state.0.lock().map_err(|e| e.to_string())?;
    db::variant_detail(&conn, &variant_id)
}
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![list_library, variant_detail])
